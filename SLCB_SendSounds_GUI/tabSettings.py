import pygame, os
import ttk, Tkinter as tk, tkFileDialog as dialog, dbFunctions as fn
...
chatOptions = ['Chat', 'Announcement (/announce)', 'Action (/me)', 'Announcement+Action (/announce /me)']
...
def createVars():
    ...
    if scriptSettings == None:
        ...
        defData = {
            'sndCommandTrigger': '#',
            'sndFolderSFX': os.path.realpath(os.path.join(os.path.dirname(__file__), '../[SLCB_SendSounds_Sounds')).lower(),
            'sndGlobalVolume': 50,
            'saveScriptLogs': True
        }
    ...
    sndCommandTrigger = tk.StringVar(value=currentSettings['sndCommandTrigger'])
...
def addCanvas(tab):
    ...
    canvas.grid(column=0, row=0, sticky=tk.W+tk.N+tk.S +tk.E, padx=10, columnspan=2, pady=10)
    ...
    scrollY.grid(row=0, column=1, padx=[0, 0], sticky=tk.N+tk.S+tk.E, pady=[20, 10])
...
def addScriptSettings(canvas, row):
    ...
    labelFrame = ttk.Labelframe(canvas, text=" Script Settings: ", takefocus=0)
...
def addDBManager(canvas, row):
    ...
    labelFrame = ttk.Labelframe(canvas, text=" Database Manager: ", takefocus=0)
    labelFrame.grid(row=row, column=0, sticky=tk.E+tk.W+tk.N+tk.S, padx=10, pady=10)
    labelFrame.config(border=1, relief=tk.SOLID)
    ...
    fixDatabaseDisable = ttk.Button(labelFrame, text='Fix database Integrity - disable mode', name='fixDisableBtn')
    ...
    fixDatabaseDelete = ttk.Button(labelFrame, text='Fix database Integrity - delete mode', name="fixDeleteBtn")
    ...
    clearDatabase = ttk.Button(labelFrame, text='Remove triggers without sound from database', name="fixClearBtn")
    clearDatabase.grid(row=5, column=0, columnspan=2, sticky=tk.E+tk.W, pady=[5,0])
    clearDatabase.configure(command=lambda mode='fix', cb=fullRecheck: fn.dbClear(mode, cb))
...
def previewSound(sound, volSetter):
    ...
    if soundFile != '':
        try:
            ...
            pass
    if msg != '':
        msg = msg.format(time=fn.dbLogger.getTime(), audio=soundFile, vol=vol, err=e)
    else:
