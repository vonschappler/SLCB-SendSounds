...
def addDBManager(canvas, row):
    ...
    if passCheck[0]:
        ...
        text = 'Some entries on the database seem to not have sound file inside the selected\nsounds folder. (Total entries found: {issuesCount})'
    ...
    if emtpyEntriesCheck > 0:
        ...
        enableDatabase.configure(state='enabled')
    else:
        emptyEntriesMsg['text'] = 'Your database do not have any disabled sounds'
        enableDatabase.configure(state='disabled')
...
def setSettings():
    ...
    scriptSettings = fn.readJson(fn.scriptSettingsFile)
    folder = scriptSettings['sndFolderSFX']
...
def selectFolder():
    ...
    if newDir != '':
        settingsVars[1].set(newDir.lower())
...
def recheckMissingFiles():
    ...
    if result > 0:
        ...
        emptyEntriesMsg['text'] = 'Your database is cleared from triggers with no sound!'
        enableDatabase.configure(state='disabled')
        clearDatabase.configure(state='disabled')
    return
