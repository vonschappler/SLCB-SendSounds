...
sys.path.append(os.path.join(os.path.dirname(__file__), './SLCB_SendSounds_Database'))
...
scriptSettingsFile = os.path.join(os.path.dirname(__file__), '[SLCB] SendSounds_settings.json')
dbSettingsFile = os.path.join(os.path.dirname(__file__), './SLCB_SendSounds_Classes/SLCB_dbSettings.json')
guiFile = os.path.realpath(os.path.join(os.path.dirname(__file__), './SLCB_SendSounds_GUI/App.py'))
...
def Execute(data):
    ...
    if data.IsChatMessage() and data.IsFromTwitch():
        ...
        if chatMsg.startswith(sndCommandTrigger):
            ...
            logMsg = logMsg.format(time=scriptLogger.getTime(), trigger=sndTrigger, user=user)
            if saveScriptLogs:
                printLog(logMsg)
    return
